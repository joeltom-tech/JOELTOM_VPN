...
if (process.env.KATASHIE_JWT_SECRET && process.env.KATASHIE_JWT_SECRET.length >= 32) {
  JWT_SECRET = process.env.KATASHIE_JWT_SECRET;
} else {
  JWT_SECRET = crypto.randomBytes(48).toString('hex');
  console.warn('[AUTH] KATASHIE_JWT_SECRET not set or too short — using ephemeral random secret. All sessions will be invalidated on restart.');
}
