...
def handle_admin_approval(call):
    ...
    if action == "approve":
        ...
        if success:
            ...
            bot.send_message(call.message.chat.id, f"❌ Échec de l'approbation de <code>{target_id}</code> : {res}", parse_mode="HTML")
    else:
...
def handle_revoke_approval(call):
    ...
    if action == "revoke":
        ...
        if success:
            ...
            bot.send_message(call.message.chat.id, f"❌ Échec de la révocation de <code>{target_id}</code> : {res}", parse_mode="HTML")
    else:
